...
- **PostHog key is not yet provisioned** — `VITE_POSTHOG_KEY` is unset in Railway today, so product analytics is fully wired but inert (same no-op pattern as Sentry above). To activate: sign up at posthog.com, create a project, add `VITE_POSTHOG_KEY` (and `VITE_POSTHOG_HOST` if not using PostHog Cloud US) to the Railway service's **build** variables, redeploy.
