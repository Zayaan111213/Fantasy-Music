...
- **Lineup snapshots (`LineupSnapshot`)**: `RosterSpot` is mutable and always reflects TODAY's lineup, so rendering a past matchup from it showed players acquired after that week and let a lineup swap retroactively rewrite history (bug of 2026-08-03). `captureLineupSnapshot()` in `src/roster/snapshot.ts` freezes one row per team/week/slot; `applyLineupSnapshot()` overlays it onto the matchup routes' `rosterSpots` and **falls back to the live roster when a week has no snapshot**, so weeks played before this shipped keep the old behavior rather than rendering empty. Captured write-once (`skipDuplicates`) at two points: the daily pipeline's first run of a week whose lineup `isLineupLocked()` (never Monday or the week-1 pre-game window, when rosters are still editable), plus a fallback at the top of `finalizeLeagueWeek()` that must stay **before** the trade/waiver steps that mutate rosters. `renewLeague()` deletes them — week numbers restart at 1 each season. `jobs/backfillLineupSnapshots.ts` is the one-off repair for weeks played before this shipped: only league week 1 is recoverable (`DraftPick` records artist *and* slot, so when no roster moved before the week closed the drafted lineup provably is the week-1 lineup); later weeks have no comparable record and are reported, not guessed. It also audits every finalized week, printing the stored matchup score beside the sum of its frozen starters. Default is a dry run; `--apply` writes. `jobs/repairUncreditedWeeks.ts` finishes the other half of the same incident: a week the crash-recovery path left `isFinalized` with no `winnerId`, no `week_result` event, and no standings credit (week 2 in both live leagues, week 10 in the demo league — all repaired 2026-08-20). It only touches a week that has BOTH a null winner and no `week_result` event; the event and the record credit are written in the same loop, so its absence proves the loop never ran, while a legitimate tie leaves a null winner on a week that does have its event. Same dry-run/`--apply` contract.
...
- **A past matchup's score and its lineup must agree**: three rules keep the number in the matchup header equal to the sum of the box score under it. (1) `updateMatchupScores()` only touches matchups with `isFinalized: false` — a settled game is history. Without this, a roster change made days later silently rewrote a finished week: a team that won a waiver in week 3 had its week-2 score recomputed off the new roster and week 2 read 124 against a lineup summing to 69 (Zaki S-R League, 2026-08). Repair tooling that deliberately rebuilds a settled week passes `{ includeFinalized: true }`. (2) While a week is open, `updateMatchupScores()` totals that week's `LineupSnapshot` when one exists — the same frozen rows `applyLineupSnapshot()` serves to the matchup routes — falling back to the live roster only for weeks that predate snapshotting. `bestArtistScore()`/`resolveWinner()` take the same `{ leagueId, week }` so the tiebreaker reads those rows too. (3) The daily pipeline does **not** re-ingest or re-score a chart week that has already closed (`isChartWeekClosed()` — Monday PT, the one day `getCurrentWeekDate()` names a finished week). Every chart week used to get one last ingest the Monday after it closed, hours *after* finalize froze the matchup score from the older values, so a finished game's box score drifted away from its own header (week 1 printed 132 under a 104 headline). A week's chart data is therefore whatever its last in-window run saw — exactly what the final score was built from. Manual CLI runs can force it with `--ingest`.
...
- `SHOT-LIVE` — "Draft Night", private, `drafting`, 10 teams, stopped 20 picks in (2 rounds done) with `shot01` on the clock at the top of round 3 (deep enough to read as a live draft, shallow enough that the available-artist list still leads with recognizable names). Picks are made through `makePick()`, not written directly, so rounds/pick numbers/roster spots match a real draft. Nothing counts down until the room is opened for the first time (no timer exists until a `draft:join` creates one), but from then on the draft drives itself: `disconnect` only clears the socket's user id and `fireAutoDraft` restarts the clock behind every expiry, so it auto-drafts a pick a minute until the board is full whether or not anyone is watching. Re-run the script immediately before shooting the room and capture inside the first 60 seconds.
