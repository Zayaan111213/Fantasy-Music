...
import { getWeekPhase, type WeekPhase } from '../utils/weekPhase';
...
function PlayersTab({ leagueId, league, onProposeTrade }: {
  ...
  // instant free agency; otherwise they queue as waiver claims for Sunday night.
      ...
      {isLoading ? (
        ...
        <Card>
          <div className="p-3 border-b border-white/10">
            {/* Column spans must stay in sync with the body rows below.
                Status gets an extra column on mobile so an owner name plus the
                trade button has room to truncate instead of spilling over the
                5W figure. */}
            <div className="grid grid-cols-12 gap-1 text-xs uppercase tracking-wider font-medium">
              ...
              <div className="col-span-4 sm:col-span-3 flex justify-end text-gray-500 min-w-0">Status</div>
            </div>
          </div>
          <div className="divide-y divide-white/5">
            {sorted.map((artist) => (
              <div key={artist.id} className="p-3 hover:bg-white/5 transition-colors">
                <div className="grid grid-cols-12 items-center gap-1">
                  ...
                  <div className="col-span-2 text-right font-mono text-sm font-semibold text-white truncate">
                    {(artist.lastWeekPoints ?? 0).toFixed(1)}
                  </div>
                  <div className="col-span-2 text-right font-mono text-sm text-gray-300 truncate">
                    {(artist.avgLast5Points ?? 0).toFixed(1)}
                  </div>
                  <div className="col-span-4 sm:col-span-3 flex justify-end items-center gap-1.5 min-w-0 overflow-hidden">
                    {artist.rosteredBy ? (
                      <>
                        {/* min-w-0 is what makes `truncate` bite: without it the
                            span keeps its full content width and, under
                            justify-end, spills left across the 5W column. */}
                        <span className="font-condensed sm:font-sans text-xs text-gray-500 truncate min-w-0">@{artist.rosteredBy.name}</span>
